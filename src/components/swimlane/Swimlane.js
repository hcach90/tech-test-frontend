...
export const Swimlane = props => (
  <div className={ 'swimlane__container' }>
    { props.lanes.map((lane, index) => (<Lane key={index} title={ lane.title } start={ props.start } end={ props.end } cards={ lane.cards } />)) }
  </div>
)
...
export const LaneDetail = props => (
  <div className={ 'swimlane__detail' }>
    { props.cards.map((card, index) => (<Card key={`card-${index}`} laneStart={ props.start } laneEnd={ props.end } className={ card.className } style={ card.style } description={ card.description } start={ card.start } end={ card.end } />)) }
  </div>
)
